    ...
    public class FOETC
    {
        ...
        public DataTable dataGridFillFromDataTable(DataGridView dgv, DataTable src, string[] columns)
        {
            ...
            try
            {
                ...
                List<(DataGridViewColumn Column, bool IsVisible, int DisplayIndex)> columnSettings = new List<(DataGridViewColumn, bool, int)>();

                // 모든 컬럼을 순회하며 설정 정보를 리스트에 저장
                for (int i = 0; i < dgv.Columns.Count; i++)
                {
                    var column = dgv.Columns[i];
                    if (column.Tag == null)
                    {
                        columnSettings.Add((column, false, dgv.Columns.Count - 1));
                    }
                    else
                    {
                        int index = Convert.ToInt32(column.Tag.ToString());
                        columnSettings.Add((column, true, index));
                    }
                }

                // 리스트를 순회하며 컬럼 설정을 적용
                foreach (var (column, isVisible, displayIndex) in columnSettings)
                {
                    ...
                    {
                        int index = Convert.ToInt32(column.Tag.ToString());
                        column.DataPropertyName = columns[index];
                    }
                }
        ...
        */
        ...
        public string lookupExtraColumnSelected(ComboBox cb, string column)
        {
            DataTable dt = (DataTable)cb.DataSource;
            int index = cb.SelectedIndex;

            if (dt == null || index == -1 || dt.Rows.Count <= index)
                return "";
